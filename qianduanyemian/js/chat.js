...
function createPostMethod(_method,options) {
	...
	return $.ajax({
		url:"http://121.41.61.101/STChat/api/api.php",
		type:"post",
		dataType:"json",
		async:"false",
...
$("[type='file']").change(
	function() {
			...
			//$("form").action = "http://file.api.weixin.qq.com/cgi-bin/media/upload?access_token="+ATOKEN+"&type=image";
			var _data = {};
			$("form").serializeArray().map(function(x){_data[x.name] = x.value;});
			$.ajax({
			...
			$.ajax(
				{
					...
					type:"post",
					data:JSON.stringify({"token":"test","base64":base64,"requestMethod":"upload"}),
